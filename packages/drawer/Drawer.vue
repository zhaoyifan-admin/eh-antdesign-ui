...
<template>
  <div class="eh-drawer" tabindex="-1">
    <Transition name="fade">
      <div v-show="open" class="eh-drawer-mask" @click.self="onBlur"></div>
    </Transition>
    <Transition :name="`motion-${placement}`">
      <div
        v-show="open"
        class="eh-drawer-wrapper"
        :class="`drawer-${placement}`"
        :style="`z-index: ${zIndex}; ${['top', 'bottom'].includes(placement) ? 'height:' + drawerHeight : 'width:' + drawerWidth};`">
        <div class="eh-drawer-content">
          <div class="eh-drawer-body-wrapper" v-if="!destroyOnClose">
          ...
          <div class="eh-drawer-body-wrapper" v-if="destroyOnClose&&open">
            <div class="eh-drawer-header">
              <div class="eh-header-title">
