...
interface Props {
  ...
  fileList?: FileType[] // (v-modal)已上传的文件列表
}
...
<template>
  <div class="eh-upload-list">
    <Space :size="gap">
      <div class="eh-upload-item" v-for="n of showUpload" :key="n">
        <div class="eh-upload">
          <div
            v-show="!uploading[n-1] && !uploadedFiles[n-1]"
            class="eh-upload-wrap"
          ...
          <div class="eh-file-uploading" v-show="uploading[n-1]">
            <Spin class="u-spin" :tip="uploadingTip" size="small" indicator="dynamic-circle"/>
          </div>
          <div class="eh-file-preview" v-if="uploadedFiles[n-1]">
            <Image
            ...
            <!-- <img class="eh-image"  :style="`object-fit: ${fit};`"  /> -->
            ...
            <div class="eh-file-mask">
              <a class="eh-icon" title="预览" @click="onPreview(n-1, uploadedFiles[n-1].url)">
              ...
              <a class="eh-icon" title="删除" @click.prevent.stop="onRemove(n-1)" v-show="!disabled">
